...
For Live Merchants: You can use the website that you have submitted in the Go-Live application. If you need to edit/change the site URL, you can send a request to ','woocommerce' ),
  ...
  function get_paytabs_args( $order ) {
      ...
      $lang_ = "Arabic";
        ...
        $paytabs_args = array(
          ...
          "address_address" =>  $order->shipping_address_1,
          'state'           =>  $order->billing_state == '' ? $order->billing_city : $order->billing_state,
          'city'            =>  $order->billing_city,
          "postal_code"     =>  $order->billing_postcode  == '' ? '0000' : $order->billing_postcode,  
          "postal_code_shipping"  => $order->billing_postcode == '' ? '0000' : $order->billing_postcode,  
        ...
        if ( 'yes' == $this->send_shipping ) {
              ...
              $paytabs_args['postal_code_shipping']= $order->shipping_postcode == '' ? '0000' : $order->shipping_postcode;
        } else {
              $paytabs_args['address_shipping']     = $order->shipping_address_1.' '.$order->shipping_address_2;          
              $paytabs_args['city_shipping']       = $order->shipping_city;
              $paytabs_args['state_shipping']      = $order->shipping_state == '' ? $order->shipping_city : $order->shipping_state;
         ...
         $_SESSION['addreess_one']=$order->billing_address_1;
         $_SESSION['shipping_addreess_one']=$order->shipping_address_1;
  ...
  if (strlen($_SESSION['addreess_one']) > 40 or strlen($_SESSION['shipping_addreess_one']) > 40  ) {
          return wc_add_notice( '<strong>Error:</strong> ' . __( 'The address should not have more than 40 characters .', 'woocommerce' ), 'error' );  
        }
  ...
  */
  function complete_transaction() {
        global $woocommerce;       
       $order = new WC_Order(WC()->session->get('order_id', 0));
  ...
  $request_string1 = http_build_query($request_string);

        $gateway_url= $this->liveurl.'/apiv2/verify_payment';
        $getdataresponse = $this->sendRequest($gateway_url, $request_string1);
        $object = json_decode($getdataresponse);
    
      if (isset($object) && !empty($object) && isset($_POST['payment_reference'])) {
    ...
    // Change the status to pending / unpaid
            ...
            // Add error for the customer when we return back to the cart
           // $message=$object->result;
            wc_add_notice( '<strong></strong> ' . __('The payment is cancelled', 'error' ), 'error' );     
            // Redirect back to the last step in the checkout process
            wp_redirect( $this->get_cancel_order_url( $order ) );
            exit;  
  }
  ...
  function getccPhone($code){
        $countries = array(
          ...
          "RO" =>'+642',// array("PARAGUAY", "PY", "PRY", "600"),
